...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("Shankar suman ❯ ") + chalk.hex("#00ffff")("fixed by Edsel Paculanang"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ Shankar suman ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/RandomlyReaper18/RandomlyReaperBypass/main/listban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/RandomlyReaper18/RandomlyReaperBypass/main/listban.json').then(dataGban => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Edsel`, '[ Edsel (Paculanang) ]');
        logger('Hey, thank you for using this Bot', '[ Edsel (Paculanang) ]');
        logger("Fixed By Edsel Paculanang", '[ Edsel (Paculanang) ]');
...
cron.schedule('0 0 0 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    ...
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Life Update:\nGood morning everyone(shankar) .", now.threadID) : '');
  });
}, {
  scheduled: true,
...
cron.schedule('0 0 4 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == false && now.threadID != list.threadID) ? loginApiData.sendMessage("Daily Update:\nGood morning group washiyon\n (shankar)", now.threadID) : '');
  });
}, {
  scheduled: false,
  timezone: "Asia/Kolkata"
...
cron.schedule('0 0 7 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == false && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon humans, let's eat.", now.threadID) : '');
  });
}, {
  scheduled: false,
  timezone: "Asia/Kolkata"
...
cron.schedule('0 30 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == false && now.threadID != list.threadID) ? loginApiData.sendMessage("Daily Update:\nkaise ho group ke tharkiyon sab😐?\nBy shankar", now.threadID) : '');
  });
}, {
  scheduled: false,
  timezone: "Asia/Kolkata"
...
cron.schedule('0 0 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == false && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon tharkiyon", now.threadID) : '');
  });
}, {
  scheduled: false,
  timezone: "Asia/Kolkata"
...
cron.schedule('0 0 18 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == false && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodevening humans, it's already 6pm, have you all eaten?", now.threadID) : '');
  });
}, {
  scheduled: false,
  timezone: "Asia/Kolkata"
...
cron.schedule('0 0 20 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == false && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodevening humans, it's already 8pm, have you all eaten?", now.threadID) : '');
  });
}, {
  scheduled: false,
  timezone: "Asia/Kolkata"
...
cron.schedule('0 0 24 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodmorning humans, another day another hope. ", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Kolkata"
