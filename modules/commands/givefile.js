...
module.exports.run = async ({ args, api, event }) => {
  ...
  const permission = ["100088309851834", ""];
  	if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command. Only Prince Sanel Osorio ", event.threadID, event.messageID);
	var path = [],
		pathrn = [],
		pathrntxt = [];
	var msg = '';
