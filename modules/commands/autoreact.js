...
module.exports.handleEvent = function({ api, event, client, __GLOBAL }) {
	...
	api.setMesaageReaction("👍", event.messageID, (err) => {}, true)
	      };
	if(react.includes("bot") || react.includes("Bot")) {
		var bot = {
			   body: ""
			}
			api.sendMessage(bot, threadID, messageID);
	api.setMessageReaction("🙂", event.messageID, (err) => {}, true);
          };  
    if(react.includes("${global.config.BOTNAME") || react.includes("sofia")) {
    	var sofia = {
    	       body: ""
           }
          api.sendMessage(sofia, threadID, messageID);
    
	api.setMessageReaction("💜", event.messageID, (err) => {}, true)
	      };
	if(react.includes("pogi ng creator") || react.includes("Pogi ni p") || react.includes("pogi ko") || react.includes("Pogi ni bot") || react.includes("pogi")) {
		var pogi = {
			   body: ""
			}
			api.sendMessage(pogi, threadID, messageID);
    api.setMessageReaction("🥺", event.messageID, (err) => {}, true)
          };
