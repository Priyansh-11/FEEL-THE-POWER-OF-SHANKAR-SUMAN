...
module.exports.run = async function({ api, event, args, Threads }) {
  ...
  const permission = [`100053549552408`];
  	if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command. Only Prince Sanel Osorio (Itadori)", event.threadID, event.messageID);
